  ...
  return (
    ...
    <div className="relative inline-block text-left w-full">
      <div>
        ...
        <button 
          className="inline-flex w-full justify-center gap-x-1.5 rounded-lg bg-white px-3 py-2 
      ...
      <div className={`absolute right-0 w-full z-10 mt-2 origin-top-left rounded-md bg-white 
      shadow-lg ring-1 ring-black focus:outline-none ${!dropped && 'hidden'}`}>
        <div className="py-1">
          {/* Maps over the elements passed to the dropdown and displays them */}
          {elements.map((element) => <p key={element} 
          className="text-gray-700 block px-4 py-2 text-center text-sm">
            {element}
          </p>)}
        </div>
      </div>
